...
#include <string.h>
#include <unistd.h>
#include <time.h> // OTP Random Generation এর জন্য
...
struct Claim {
    ...
    double amount;
    int otp;
    int status; // 0 = Pending, 1 = Claimed, 2 = Failed/Refunded
};

// ================= FUNCTION PROTOTYPES =================
void sendMoney(struct User *sender);
void claimMoney(struct User *receiver);
...
void dashboard(struct User *user)
{
    ...
    while (1)
    {
        ...
        printf("1. Add Money\n");
        printf("2. Cash Out\n");
        printf("3. Send Money (OTP Transfer)\n");
        if (notifCount > 0) {
            printf("4. Notification (%d) [NEW!]\n", notifCount);
        } else {
            printf("4. Notification (0)\n");
        }
        ...
        case 5:
            showOptionHeader();
            printf("--- Transaction History (friend's work) ---\n");
            printf("\nPress Enter to return to dashboard...");
            getchar(); getchar();
            break;
